...
/// Build a table of all the requests.
    ...
    foreach ($pending as $course) {
        ...
        $requesterfullname = fullname($course->get_requester(), has_capability('moodle/site:viewfullnames', $PAGE->context));
        if (user_can_view_profile($course->get_requester())) {
            $requesterfullname = html_writer::link(new moodle_url('/user/profile.php', [
                'id' => $course->get_requester()->id,
            ]), $requesterfullname);
        }
